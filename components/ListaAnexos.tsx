  ...
  return (
    <div className="space-y-2 mt-2">
      {anexos.map((anexo) => (
        <div
            key={anexo.id}
            className="group flex items-center justify-between p-2 rounded-lg bg-surface border border-transparent hover:border-primary-hover/40 hover:bg-surface-highlight/50 transition-all"
        >
            <Link
                href={anexo.url}
                target="_blank"
            ...
            >
                ...
                <div className="flex flex-col min-w-0">
                    <span className="text-sm font-medium text-foreground truncate group-hover:text-primary transition-colors" title={anexo.nome}>
                        {anexo.nome}
                    </span>
