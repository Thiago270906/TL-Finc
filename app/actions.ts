...
import type { LinhaConfirmacaoImportacao } from '@/lib/extrato/tipos'
...
// =============================================================================
// IMPORTAÇÃO INTELIGENTE DE EXTRATOS (PDF)
// =============================================================================

// A análise do PDF em si (upload de arquivo) roda em app/api/extrato/analisar/route.ts —
// Server Actions mostraram-se pouco confiáveis para upload de arquivo em navegadores mobile.

export async function confirmarImportacaoExtrato(
  bancoId: string,
  linhas: LinhaConfirmacaoImportacao[]
): Promise<ActionResult<{ criados: number; conciliados: number; saldo_atual: number }>> {
  try {
    const usuario = await getUsuarioLogado()
    if (!usuario) return { success: false, error: 'Não autenticado.' }
    ...
    for (const l of linhasValidas) {
      if (l.acao === 'CRIAR' && !l.planoContasId) {
        return { success: false, error: `Selecione a categoria da linha "${l.descricao}" antes de importar.` }
      }
      if (l.acao === 'CONCILIAR' && !l.lancamentoConciliadoId) {
        return { success: false, error: `A linha "${l.descricao}" está marcada para conciliar, mas não tem um lançamento vinculado.` }
      }
    }

    linhasValidas.sort((a, b) => a.data.localeCompare(b.data))
    ...
    await prisma.$transaction(async (tx) => {
      let saldoAtual = Number(banco.saldo_atual)
      const usados = new Set<string>()

      for (const linha of linhasValidas) {
        const delta = linha.tipoMovimento === 'ENTRADA' ? linha.valor : -linha.valor
        const saldoAnterior = saldoAtual
        const saldoPosterior = Math.round((saldoAtual + delta) * 100) / 100
        const dataMovimento = new Date(linha.data)
        ...
        if (linha.acao === 'CONCILIAR' && linha.lancamentoConciliadoId) {
          ...
          const novoLancamento = await tx.lancamentoFinanceiro.create({
            data: {
              ...
              valor: linha.valor,
              dt_vencimento: dataMovimento,
              dt_pagamento: dataMovimento,
              status: 'PAGO',
              plano_contas_id: linha.planoContasId,
              banco_id: bancoId,
              saldo_banco_anterior: saldoAnterior,
              saldo_banco_posterior: saldoPosterior,
            },
    ...
    revalidatePath('/financeiro/contas-a-receber')
    revalidatePath('/financeiro/balancete')
    return { success: true, data: { criados, conciliados, saldo_atual: saldoFinal } }
  } catch (erro) {
    console.error('Erro ao confirmar importação de extrato:', erro)
    return { success: false, error: 'Erro ao importar o extrato.' }
  }
}
